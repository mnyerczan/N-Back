...
require_once 'DB/baseDbApi.php';
require_once 'DB/entityGateway.php';

$seria = new Seria( EntityGateway::getDB(), $user->id );
...
<div class="main_header_style" >
	<div id="header_width">
		...
		<div id='seria_text' title= "<?= $seria->seria, $seria->seria > 1 ? " days series" : " day series" ?>">
			<img src="img/fat_flame_red.png" id="seria_flame_img">
		...
		<?php if( $user->id !== 1 ):?>				
			<a href="index.php?exit=1" onclick="return confirm('Are you sure?')">
				<button class="header_button">  Logout
					<img src="img/logout_white.png" class="header_button_img" style="width:20px;height:17px;">
				</button>
			</a>
			...
			<div class="border"></div>
				<a href="index.php?index=6" >
					<button class="header_button" title="Go to profile"><b id="name_marker">										
						<?=$user->name?><img src="users/<?=explode('_', $user->file_name)[0]?>/<?= $user->file_name?>"
			...
			<?php if($user->privilege == $admin_privilege):?>						
				<a href='index.php?index=6&c=1'>
					<button class='header_button'> New account
						<img src='img/add_user_blue.png' class='header_button_img' style='width:20px;height:17px;'>
					</button>
				</a>
				<div class='border'></div>
			<?php endif ?>
		...
		<?php if( !$user->login_datetime && (!isset($_GET['index']) || $_GET['index'] != '1' )): ?>		
			<a href='index.php?index=1'>
				<button class='header_button'>  Sign in
					<img src='img/login_white.png' class='header_button_img' style='width:20px;height:17px;'>
				</button>
			</a>
			<div class='border'></div>
		<?php endif ?>
