...
if(isset($case)){
	...
	switch($case)
	{
		case 'nbackDatas_modify'			: $exit = Update_nback_datas() == 0 ? '4' : 'error'; break;
...
if($case != ''){
	...
	switch((int)$exit){
		...
		case 6:header('Location: index.php?index=6&uid='.$userID);															break;
		case 9:header('Location: index.php?index=9&rep_common='.$e);														break;
		case 10:header('Location: index.php?index=6&uid='.$userID);														break;
