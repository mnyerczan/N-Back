...
if(isset($_GET['count']) && isset($_GET['rid'])){
	...
	$sql_check = 'SELECT count(*) t FROM logs l, users u where u.id = l.user_id and  menu_id="'.$_GET['rid'].'";';
...
elseif(isset($_GET['get_logs'])){
	...
	$sql_query =  'SELECT l.title,l.id as logid, u.id AS user_id,u.name AS username,l.content AS msg, l.timestamp, online, u.file_name FROM users u, logs l WHERE u.id = l.user_id and l.menu_id = '.$menu_id.' ORDER BY l.id desc LIMIT '.$limit.' OFFSET '.$offset.';';
	$get_logs = $_GET['get_logs'];

	$result1=mysqli_query($conn, $sql_query);


	if(count($get_menus_name = sql_query("SELECT name, privilege FROM menus WHERE id = ".$menu_id." and parent_id = '00000001';")) > 0 ){
